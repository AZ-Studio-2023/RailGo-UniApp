		...
		<view class="ux-padding-large">
			<view class="ux-mt-large">
				...
				<view class="ux-flex" style="align-items: center; margin-bottom: 10px;">
					...
					<uni-easyinput placeholder="请输入公测码" @input="inputCode"
						style="min-width: 200px; flex-grow: 1;"></uni-easyinput>
				</view>
				<view class="ux-flex " style="align-items: center;">
					<text style="width: 80px; text-align: right;" class="">QQ号：</text>
					<uni-easyinput placeholder="请输入QQ号" @input="inputQQ"
						style="min-width: 200px; flex-grow: 1;"></uni-easyinput>
				</view>
			</view>
			<br>
			<br>
			<br>
			<button class="primary-button ux-mt-large" style="position: fixed; right: 20px; " @click="check"
				hover-class="">
	...
	export default {
		...
		methods: {
			...
			async check() {
				...
				this.load = true;
				try {
					const Response = await uniGet("https://auth.railgo.zenglingkun.cn/api/check/" + this.ver +
						"?userid=" + this.qq + "&key=" + this.code);
